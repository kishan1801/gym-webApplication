...
const OrderConfirmationPage = () => {
  ...
  useEffect(() => {
    ...
    socket = io('https://fitlyfy.onrender.com', {
  ...
  const refreshOrderStatus = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem(`fitlyf_order_${orderId}`, JSON.stringify(response.data.data));
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <span className="text-yellow-400 text-sm">
                  {connectionStatus === 'disconnected' ? 'Connection lost. Updates may be delayed.' :
        ...
        <div className="text-center mb-12">
          <div className={`w-20 h-20 ${
            ...
            currentStatus === 'shipped' ? 'bg-blue-600' :
            currentStatus === 'processing' ? 'bg-purple-600' :
            'bg-brand-primary'
          ...
          <h1 className="text-4xl font-bold text-white mb-4">
            {currentStatus === 'delivered' ? 'Order Delivered!' :
             ...
             currentStatus === 'processing' ? 'Order Processing!' :
             'Order Confirmed!'}
          </h1>
          <p className="text-gray-400 text-lg mb-2">
            Thank you for your purchase, {order.customer?.firstName || 'Customer'}!
          </p>
          <p className="text-gray-500">
            Your order ID: <span className="text-brand-primary font-bold">{order.orderId}</span>
          </p>
          ...
          <div className="mt-4 inline-flex items-center px-4 py-2 rounded-full bg-gray-800/50">
            ...
            <span className="text-sm text-gray-400">
              {connectionStatus === 'connected' ? 'Live updates active' : 'Manual refresh required'}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-dark-card rounded-xl p-6 mb-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white">Order Status Timeline</h2>
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                  currentStatus === 'delivered' ? 'bg-green-900/30 text-green-400' :
                  currentStatus === 'shipped' ? 'bg-blue-900/30 text-blue-400' :
                  currentStatus === 'processing' ? 'bg-purple-900/30 text-purple-400' :
                  ...
                  {statusSteps.map((step, index) => (
                    <div key={step.id} className="relative flex items-start">
                      <div className={`relative z-10 flex items-center justify-center w-12 h-12 rounded-full ${
                        step.completed ? 'bg-brand-primary' :
              ...
              <div className="space-y-4">
                {order.items?.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-4 bg-gray-800/30 rounded-lg">
                    <div className="flex items-center">
                      <img
                        src={item.image || 'https://images.unsplash.com/photo-1593095948071-474c5cc2989d?w-400&q=80'}
                        alt={item.name}
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-gray-400 text-sm">₹{item.price?.toLocaleString() || '0'} each</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-dark-card rounded-xl p-6">
                <h3 className="text-xl font-semibold text-white mb-4">Shipping Details</h3>
                <div className="space-y-3">
                  ...
                  <div className="flex space-x-6">
                    <div>
                      ...
                      <p className="text-gray-300">📧 {order.customer?.email || 'No email'}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Phone</p>
                      <p className="text-gray-300">📱 {order.customer?.phone || 'No phone'}</p>
                    </div>
                  </div>
                  {order.shipping?.trackingNumber && (
                    <div className="pt-3 border-t border-gray-800">
                      <p className="text-gray-400 text-sm">Tracking Number</p>
                      <p className="text-white font-mono">{order.shipping.trackingNumber}</p>
                      {order.shipping?.courier && (
                        <p className="text-gray-300 text-sm mt-1">Courier: {order.shipping.courier}</p>
                      )}
                    </div>
                  )}
                </div>
                  ...
                  <div>
                    ...
                    <p className="text-white text-lg">
                      {order.payment?.method === 'cod' ? '💵 Cash on Delivery' :
                       order.payment?.method === 'razorpay' ? '💳 Card/UPI Payment' : 
                       order.payment?.method === 'online' ? '💳 Online Payment' : '👛 Wallet'}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className={`text-lg font-bold ${
                      ...
                      order.payment?.status === 'failed' ? 'text-red-400' : 'text-gray-400'
                    }`}>
                      {order.payment?.status?.charAt(0).toUpperCase() + order.payment?.status?.slice(1) || 'Pending'}
                    </p>
                  </div>
                  <div>
          ...
          <div>
            <div className="bg-dark-card rounded-xl p-6 sticky top-24">
              <h2 className="text-2xl font-bold text-white mb-6">Order Actions</h2>
              
              <div className="space-y-6">
                {/* Status Updates Info */}
                <div className="p-4 bg-gray-800/30 rounded-lg">
                  <h4 className="text-white font-semibold mb-2">🔄 Live Updates</h4>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <span className="text-gray-300 text-sm">
                      {order.updatedAt ? formatDate(order.updatedAt) : 'Just now'}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-gray-800">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center">
                      ...
                      <span className="text-gray-300 text-sm">support@fitlyf.com</span>
                    </div>
                    <div className="flex items-center">
                      <span className="text-gray-500 mr-3">📱</span>
                    ...
                    <div className="flex items-center">
                      <span className="text-gray-500 mr-3">🕒</span>
                      <span className="text-gray-300 text-sm">Mon-Sun: 8AM-10PM</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {order.estimatedDelivery && (
          <div className="mt-8 p-6 bg-gradient-to-r from-blue-900/20 to-purple-900/20 rounded-xl">
            <div className="flex flex-col md:flex-row items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-white mb-2">Estimated Delivery</h3>
                <p className="text-gray-300">
                  ...
                  <div>
                    ...
                    <p className="text-white font-semibold">
                      {currentStatus === 'delivered' ? 'Delivered' :
                       currentStatus === 'shipped' ? 'In Transit' :
                       currentStatus === 'processing' ? 'Preparing for Shipment' :
                       'Order Confirmed'}
                    </p>
                  </div>
                </div>
