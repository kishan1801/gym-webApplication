...
const getImageUrl = (imagePath) => {
  ...
  const transformTrainerData = (trainer) => {
    return {
      ...
      bio: trainer.bio || `${trainer.name} is a certified ${trainer.specialization.toLowerCase()} trainer with ${trainer.experience} years of experience.`,
      achievements: [
      ...
      trainingStyle: trainer.availability ? `Available: ${trainer.availability}` : 'Flexible scheduling available',
      quote: getTrainerQuote(trainer.specialization),
      image: getImageUrl(trainer.profileImage),
      rating: trainer.ratings?.average || 4.5,
      clients: trainer.ratings?.totalReviews || Math.floor(Math.random() * 200) + 50,
  ...
  const getTrainerQuote = (specialization) => {
    const quotes = {
      ...
      'Cardio': 'Your body can stand almost anything. It\'s your mind you have to convince.',
      'Strength Training': 'Strength doesn\'t come from what you can do. It comes from overcoming the things you once thought you couldn\'t.',
      'CrossFit': 'Champions aren\'t made in gyms. Champions are made from something they have deep inside them.',
      'Pilates': 'Physical fitness is the first requisite of happiness.',
      'Zumba': 'Dance like nobody\'s watching, but make sure you\'re burning calories while doing it!',
      'MMA': 'The pain you feel today will be the strength you feel tomorrow.',
      'Swimming': 'In the water, you find freedom. In training, you find discipline.',
      'Dance': 'Movement is a medicine for creating change in a person\'s physical, emotional, and mental states.',
      'Nutrition': 'Good nutrition is the foundation of fitness. You can\'t out-train a bad diet.',
      'Weight Loss': 'Transformation is not a future event. It is a present activity.',
      'Senior Fitness': 'Age is just a number. Fitness is for everyone.'
    };
    return quotes[specialization] || 'Fitness is not about being better than someone else. It\'s about being better than you used to be.';
  };

  // Helper function to get specialties based on specialization
  const getSpecialties = (specialization) => {
    const specialtiesMap = {
      'Yoga': ['Yoga Therapy', 'Mobility Training', 'Stress Management', 'Posture Correction', 'Mindfulness'],
      'Cardio': ['Fat Loss', 'Metabolic Conditioning', 'Endurance Training', 'Group Fitness', 'HIIT'],
      'Strength Training': ['Powerlifting', 'Olympic Lifting', 'Athletic Performance', 'Functional Training', 'Muscle Building'],
      'CrossFit': ['Functional Fitness', 'High-Intensity Training', 'Gymnastics', 'Weightlifting', 'Competition Prep'],
      'Pilates': ['Core Strength', 'Posture Improvement', 'Rehabilitation', 'Flexibility', 'Injury Prevention'],
      'Zumba': ['Dance Fitness', 'Cardio Dance', 'Latin Dance', 'Group Coordination', 'Rhythm Training'],
      'MMA': ['Combat Sports', 'Self-Defense', 'Conditioning', 'Technique Training', 'Fight Prep'],
      'Swimming': ['Swim Technique', 'Endurance Swimming', 'Water Safety', 'Competitive Swimming', 'Triathlon Training'],
      'Dance': ['Choreography', 'Rhythm Training', 'Dance Styles', 'Performance Skills', 'Flexibility'],
      'Nutrition': ['Weight Management', 'Sports Nutrition', 'Meal Planning', 'Dietary Guidance', 'Metabolic Health'],
      'Weight Loss': ['Body Transformation', 'Metabolic Conditioning', 'Nutrition Coaching', 'Accountability', 'Lifestyle Change'],
      'Senior Fitness': ['Mobility Training', 'Balance Improvement', 'Strength Building', 'Health Optimization', 'Gentle Exercise']
    };
    return specialtiesMap[specialization] || [specialization, 'Personal Training', 'Fitness Coaching', 'Goal Setting', 'Progress Tracking'];
  };
  ...
  const filteredTrainers = trainers
    .filter(trainer => {
      if (activeSpecialty !== 'all') {
        const specId = trainer.specialization.toLowerCase().replace(/\s+/g, '-');
  ...
  const calculateStats = () => {
    if (trainers.length === 0) return { avgExperience: 0, totalCerts: 0, totalClients: 0, avgRating: 0 };
    
    const avgExperience = Math.round(trainers.reduce((sum, t) => sum + t.experience, 0) / trainers.length);
    const totalCerts = trainers.reduce((sum, t) => sum + (t.certifications?.length || 0), 0);
    const totalClients = trainers.reduce((sum, t) => sum + (t.ratings?.totalReviews || 0), 0);
    const avgRating = trainers.reduce((sum, t) => sum + (t.ratings?.average || 0), 0) / trainers.length;
    
    return { avgExperience, totalCerts, totalClients, avgRating: avgRating.toFixed(1) };
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 sm:mb-16">
            ...
            <h2 className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl font-black text-white mb-4 leading-tight">
              Elite <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary via-white to-brand-secondary">Training Experts</span>
            </h2>
          ...
          <div className="flex justify-center py-20">
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm mt-4">Loading expert trainers...</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="max-w-lg mx-auto">
            <div className="relative bg-gradient-to-br from-red-900/20 to-red-950/20 backdrop-blur-sm border border-red-800/50 rounded-2xl p-8 overflow-hidden">
              ...
              <div className="relative text-center">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Unable to Load Trainers</h3>
          ...
          <div className="max-w-md mx-auto">
            <div className="relative bg-gradient-to-br from-gray-900/50 to-gray-950/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-8 overflow-hidden">
              <div className="relative text-center">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Trainers Coming Soon</h3>
                <p className="text-gray-400 text-sm mb-6">We're assembling an elite team of fitness professionals</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className=" sm:py-10 md:py-10 bg-gradient-to-b from-gray-900 to-gray-950">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12 sm:mb-16">
            <div className="inline-flex items-center justify-center w-16 h-1 bg-gradient-to-r from-brand-primary to-brand-secondary mb-6"></div>
            <h2 className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl font-black text-white mb-4 leading-tight">
              Meet Our <span className="text-transparent bg-clip-text bg-gradient-to-r  text-white from-brand-primary via-white to-brand-secondary">Elite Trainers</span>
            </h2>
            
            <p className="text-gray-400 text-lg sm:text-xl max-w-3xl mx-auto font-light leading-relaxed">
              Certified professionals with proven expertise and passion for transforming lives through personalized fitness guidance
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-16">
            {[
              { label: 'Years Avg Experience', value: `${stats.avgExperience}+`, icon: '📅' },
              { label: 'Total Certifications', value: `${stats.totalCerts}+`, icon: '🏆' },
            ...
            <div className="flex flex-col lg:flex-row gap-6 mb-8">
              ...
              <div className="lg:w-48">
                <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl px-6 py-4 text-center">
                  <div className="text-white text-2xl font-bold">{filteredTrainers.length}</div>
              ...
              >
                ...
                <div className="relative h-64 sm:h-72 overflow-hidden">
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      e.target.src = 'https://images.unsplash.com/photo-1534367507877-0edd93bd013b?w=800&q=80';
                    }}
                  />
                  ...
                  <div className="absolute bottom-4 left-4">
                    <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${
                      ...
                      {trainer.category === 'expert' ? 'EXPERT' : trainer.category === 'senior' ? 'SENIOR' : 'PRO'}
                    </span>
                  </div>
                  ...
                  <div className="absolute bottom-4 right-4 text-right">
                    ...
                    <p className="text-brand-primary text-sm">{trainer.title}</p>
                  </div>
                </div>
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Specialty</p>
                      <p className="text-white font-semibold text-base">{trainer.specialty}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Experience</p>
                      <p className="text-white font-semibold text-base">{trainer.experience}</p>
                    </div>
                  </div>

                  <p className="text-gray-300 text-sm mb-4 line-clamp-2 leading-relaxed">{trainer.bio}</p>

                  {/* Certifications */}
                  {trainer.certifications.length > 0 && (
                    <div className="mb-4">
                      <p className="text-gray-400 text-xs uppercase tracking-wider mb-2">Credentials</p>
                      <div className="flex flex-wrap gap-1">
                        {trainer.certifications.slice(0, 2).map((cert, index) => (
                          <span key={index} className="text-xs bg-brand-primary/20 text-brand-primary px-2 py-1 rounded">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/50 to-transparent">
                    <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 md:p-10">
                      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
                        <div>
                          <h3 className="text-3xl sm:text-4xl font-bold text-white mb-2">{selectedTrainer.name}</h3>
                          <p className="text-brand-primary text-xl sm:text-2xl">{selectedTrainer.title}</p>
                          <div className="flex items-center gap-3 mt-4">
                            <div className="bg-black/50 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2">
                              <span className="text-yellow-500">★</span>
                              <span className="text-white font-bold">{selectedTrainer.rating}</span>
                              <span className="text-gray-400">({selectedTrainer.clients} clients)</span>
                            </div>
                            <div className="bg-black/50 backdrop-blur-sm px-4 py-2 rounded-xl">
                              <span className="text-white">{selectedTrainer.experience} Experience</span>
                            </div>
                          </div>
                        </div>
                        <div className="text-right">
                ...
                <div className="p-6 sm:p-8 md:p-10">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                      {/* Bio */}
                      <div>
                        <h4 className="text-xl font-bold text-white mb-4">About {selectedTrainer.name.split(' ')[0]}</h4>
                        <p className="text-gray-300 text-base leading-relaxed">{selectedTrainer.bio}</p>
                      </div>

                      {/* Philosophy */}
                      <div>
                        <h5 className="text-white font-bold text-lg mb-3">Training Philosophy</h5>
                        <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6">
                          <p className="text-gray-300 italic text-lg mb-2">"{selectedTrainer.quote}"</p>
                          <p className="text-gray-400 text-sm">{selectedTrainer.trainingStyle}</p>
                        </div>
                      </div>

                      {/* Qualifications */}
                      <div>
                        <h5 className="text-white font-bold text-lg mb-4">Key Qualifications</h5>
                    ...
                    <div className="space-y-8">
                      {/* Specialties */}
                      <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6">
                        <h5 className="text-white font-bold text-lg mb-4">Areas of Expertise</h5>
                        <div className="flex flex-wrap gap-2">
                          {selectedTrainer.specialties.map((specialty, index) => (
                            <span key={index} className="bg-gradient-to-r from-brand-primary/20 to-brand-secondary/20 text-white px-3 py-2 rounded-xl text-sm font-medium">
                              {specialty}
                            </span>
                          ))}
                        </div>
                      ...
                      {selectedTrainer.certifications.length > 0 && (
                        <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6">
                          <h5 className="text-white font-bold text-lg mb-4">Certifications</h5>
                          <ul className="space-y-3">
                            {selectedTrainer.certifications.map((cert, index) => (
                              <li key={index} className="flex items-center gap-3">
                                ...
                                <span className="text-gray-300 text-sm flex-1">{cert}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
                      ...
                      {/* Contact Info */}
                      <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6">
                        <h5 className="text-white font-bold text-lg mb-4">Contact Information</h5>
                        <div className="space-y-4">
                          <div>
                            <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Email</p>
                            <p className="text-white text-base break-all">{selectedTrainer.email}</p>
                          </div>
                          <div>
                            <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Phone</p>
                            <p className="text-white text-base">{selectedTrainer.phone}</p>
                          </div>
                          <div>
                            <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Availability</p>
                            <p className="text-white text-base">{selectedTrainer.availability}</p>
                          </div>
                        </div>
                      </div>

                      {/* Social Links */}
                      {(selectedTrainer.social.instagram || selectedTrainer.social.twitter || selectedTrainer.social.linkedin) && (
                        <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6">
                          <h5 className="text-white font-bold text-lg mb-4">Connect</h5>
          ...
          <div className="mb-12 sm:mb-16">
            <div className="text-center mb-12 sm:mb-16">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-4 sm:mb-6">
                Why Train With <span className="text-transparent bg-clip-text bg-gradient-to-r text-white from-brand-primary to-brand-secondary">Our Experts</span>
              </h3>
              <p className="text-gray-400 text-lg sm:text-xl max-w-3xl mx-auto">
              ...
              {[
                {
                  ...
                  description: "All trainers hold international certifications and undergo continuous education to stay at the forefront of fitness science.",
                  icon: "🏅",
                ...
                },
                {
                  ...
                  description: "Track record of successful transformations across diverse fitness goals with measurable, sustainable results.",
                  icon: "📈",
                ...
                },
                {
                  ...
                  description: "Customized training plans based on comprehensive individual assessment and evolving goals.",
                  icon: "🎯",
